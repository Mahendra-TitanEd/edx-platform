...
ReverifyInfo = namedtuple(
    "ReverifyInfo", "course_id course_name course_number date status display"
)
...
def csrf_token(context):
    ...
    return HTML(
        ...
        ' name="csrfmiddlewaretoken" value="{}" /></div>'
    ).format(Text(token))
...
def compose_activation_email(
    ...
    message_context.update(
        {
            "confirm_activation_link": _get_activation_confirmation_link(
                message_context["key"], redirect_url
            ),
...
def compose_and_send_activation_email(
    user, profile, user_registration=None, redirect_url=None
):
    """
    ...
    msg = compose_activation_email(
        user, user_registration, route_enabled, profile.name, redirect_url
    )
...
def disable_account_ajax(request):
    ...
    try:
        ...
        if account_action == "disable":
            ...
            context["message"] = _("Successfully disabled {}'s account").format(
                username
            )
            log.info("%s disabled %s's account", request.user, username)
        elif account_action == "reenable":
            user_account.account_status = UserStanding.ACCOUNT_ENABLED
            context["message"] = _("Successfully reenabled {}'s account").format(
                username
            )
...
def user_signup_handler(sender, **kwargs):  # pylint: disable=unused-argument
    ...
    if "created" in kwargs and kwargs["created"]:
        ...
        if site:
            ...
            log.info(
                'user {} originated from a white labeled "Microsite"'.format(
                    kwargs["instance"].id
                )
            )
...
def activate_account(request, key):
    ...
    activated_or_confirmed = (
        "confirmed" if settings.MARKETING_EMAILS_OPT_IN else "activated"
    )
    ...
    try:
        ...
        messages.error(
            request, invalid_message, extra_tags="account-activation aa-icon"
        )
    else:
    ...
    response = (
        ...
        if redirect_url and is_enterprise_learner(request.user)
...
def validate_new_email(user, new_email):
    ...
    try:
        ...
        raise ValueError(
            _("Valid e-mail address required.")
...
def validate_secondary_email(user, new_email):
    ...
    if hasattr(user, "account_recovery"):
        if (
            user.account_recovery.is_active
            and new_email == user.account_recovery.secondary_email
        ):
            raise ValueError(_("Old email is the same as the new email."))
...
def do_email_change_request(
    user, new_email, activation_key=None, secondary_email_change_request=False
):
    """
    ...
    if secondary_email_change_request:
        ...
        confirm_link = reverse(
            "activate_secondary_email", kwargs={"key": activation_key}
        )
    else:
    ...
    try:
        ...
        log.error(
            'Unable to send email activation link to user from "%s"',
            from_address,
            exc_info=True,
        )
        raise ValueError(
...
def activate_secondary_email(request, key):
    ...
    try:
        account_recovery.update_recovery_email(
    ...
    with transaction.atomic():
        ...
        try:
            ...
            response = render_to_response(
                "email_change_failed.html", {"email": user.email}
            )
            transaction.set_rollback(True)
            return response
