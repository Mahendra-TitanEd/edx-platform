...
def check_verify_status_by_course(user, course_enrollments):
    ...
    for enrollment in course_enrollments:
        ...
        if enrollment.mode in CourseMode.VERIFIED_MODES:
            ...
            if (
                relevant_verification is not None
                and relevant_verification.status == "approved"
            ):
                recent_verification_datetime = max(
                    recent_verification_datetime
                    if recent_verification_datetime is not None
            ...
            submitted = (
                relevant_verification is not None
                and relevant_verification.status not in ["created", "ready"]
            )
            if status is None and not submitted:
                if deadline is None or deadline > datetime.now(UTC):
                    if (
                        IDVerificationService.user_is_verified(user)
                        and verification_expiring_soon
                    ):
    ...
    if recent_verification_datetime:
        for key, value in status_by_course.items():  # pylint: disable=unused-variable
            ...
            ] = recent_verification_datetime.strftime("%m/%d/%Y")
...
def get_redirect_url_with_host(root_url, redirect_to):
    ...
    if not netloc:
        ...
        redirect_to = urllib.parse.urlunsplit(
            (parse_root_url.scheme, parse_root_url.netloc, path, query, fragment)
        )
...
def get_next_url_for_login_page(request, include_host=False):
    ...
    if any(param in request_params for param in POST_AUTH_PARAMS):
        ...
        redirect_to = "{}?{}".format(
            reverse("finish_auth"), urllib.parse.urlencode(params)
        )
    ...
    tpa_hint = configuration_helpers.get_value(
        "THIRD_PARTY_AUTH_HINT", settings.FEATURES.get("THIRD_PARTY_AUTH_HINT", "")
    )
    if tpa_hint:
        ...
        (scheme, netloc, path, query, fragment) = list(
            urllib.parse.urlsplit(redirect_to)
        )
...
def _get_redirect_to(request_host, request_headers, request_params, request_is_https):
    ...
    accepts_text_html = any(
        mime_type in header_accept for mime_type in ["*/*", "text/*", "text/html"]
    )
...
def _cert_info(user, enrollment, cert_status):
    ...
    if (
        status != default_status
        and course_overview.end_of_course_survey_url is not None
    ):
        status_dict.update(
            {
                "show_survey_button": True,
                "survey_url": process_survey_link(
                    course_overview.end_of_course_survey_url, user
                ),
            }
        )
    else:
                ...
                status_dict.update(
                    {
                        ...
                        "cert_web_view_url": get_certificate_url(
                            course_id=course_overview.id, uuid=cert_status["uuid"]
                        ),
                    }
                )
    ...
    if status in {
        ...
        persisted_grade = CourseGradeFactory().read(
            user, course=course_overview, create_if_needed=False
        )
        ...
        cert_gen_enabled = (
            has_self_generated_certificates_enabled(course_overview.id)
            or auto_certificate_generation_enabled()
        )
        passing_grade = persisted_grade and persisted_grade.passed
        if (
            status_dict["status"] != CertificateStatuses.downloadable
            and cert_gen_enabled
            and passing_grade
...
def do_create_account(form, custom_form=None):
    ...
    user = User(
        username=proposed_username, email=form.cleaned_data["email"], is_active=False
    )
    ...
    try:
        ...
        if username_exists_or_retired(
            ...
            raise AccountValidationError(  # lint-amnesty, pylint: disable=raise-missing-from
                _("An account with the Email '{email}' already exists.").format(
                    email=user.email
                ),
                field="email",
    ...
    profile = UserProfile(
        user=user, **{key: form.cleaned_data.get(key) for key in profile_fields}
    )
