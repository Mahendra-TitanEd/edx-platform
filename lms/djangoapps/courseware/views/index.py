...
class CoursewareIndex(View):
    ...
    def get(self, request, course_id, chapter=None, section=None, position=None):
        ...
        try:
            ...
            with modulestore().bulk_operations(self.course_key):
                ...
                if (
                    not self.is_staff
                    and self.enable_unenrolled_access
                    and self.course.course_visibility == COURSE_VISIBILITY_PUBLIC
                    and not CourseEnrollment.is_enrolled(request.user, self.course_key)
                ):
            ...
            return CourseTabView.handle_exceptions(
                request, self.course_key, self.course, exception
            )
    ...
    def microfrontend_url(self):
        ...
        try:
            unit_key = UsageKey.from_string(
                self.request.GET.get("activate_block_id", "")
            )
    ...
    def render(self, request):
        ...
        if not request.user.is_authenticated:
            ...
            allow_anonymous = check_public_access(
                self.course, [COURSE_VISIBILITY_PUBLIC]
            )
        ...
        return render_to_response(
            "courseware/courseware.html", self._create_courseware_context(request)
        )
    ...
    def _redirect_if_not_requested_section(self):
        ...
        """
        if self.chapter.url_name != self.original_chapter_url_name or (
            self.original_section_url_name
            and self.section.url_name != self.original_section_url_name
        ):
    ...
    def _clean_position(self):
        ...
        if self.position is not None:
            try:
                ...
                raise Http404(
                    f"Position {self.position} is not an integer!"
    ...
    def _find_chapter(self):
        ...
        return self._find_block(
            self.course, self.chapter_url_name, "chapter", CONTENT_DEPTH - 1
        )
    ...
    def _prefetch_and_bind_section(self):
        ...
        self.section = modulestore().get_item(
            self.section.location, depth=None, lazy=False
        )
    ...
    def _create_courseware_context(self, request):
        ...
        show_search = settings.FEATURES.get("ENABLE_COURSEWARE_SEARCH") or (
        ...
        courseware_context = {
            ...
            "disable_optimizely": not LegacyWaffleSwitchNamespace("RET").is_enabled(
                "enable_optimizely_in_courseware"
            ),
            "section_title": None,
            "sequence_title": None,
            "disable_accordion": not DISABLE_COURSE_OUTLINE_PAGE_FLAG.is_enabled(
                self.course.id
            ),
        ...
        ] = CourseSockFragmentView().render_to_fragment(request, course=self.course)
    ...
    def _add_sequence_title_to_context(self, courseware_context):
        ...
        courseware_context["sequence_title"] = display_items[
            self.section.position - 1
    ...
    def _create_section_context(
        self, previous_of_active_section, next_of_active_section
    ):
        """
        ...
        def _compute_section_url(section_info, requested_child):
            ...
            return "{url}?child={requested_child}".format(
                url=reverse(
                    ...
                    args=[
                        str(self.course_key),
                        section_info["chapter_url_name"],
                        section_info["url_name"],
                    ],
                ),
        ...
        section_context = {
            ...
            "progress_url": reverse(
                "progress", kwargs={"course_id": str(self.course_key)}
            ),
            "user_authenticated": self.request.user.is_authenticated,
            "position": position,
        }
        if previous_of_active_section:
            section_context["prev_url"] = _compute_section_url(
                previous_of_active_section, "last"
            )
        if next_of_active_section:
            section_context["next_url"] = _compute_section_url(
                next_of_active_section, "first"
            )
        # sections can hide data that masquerading staff should see when debugging issues with specific students
        section_context[
            "specific_masquerade"
        ] = self._is_masquerading_as_specific_student()
...
def save_positions_recursively_up(
    user, request, field_data_cache, xmodule, course=None
):
    """
