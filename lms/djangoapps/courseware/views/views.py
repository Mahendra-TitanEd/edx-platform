...
def _earned_but_not_available_cert_data(cert_downloadable_status):
    return CertData(
        ...
        download_url=None,
        cert_web_view_url=None,
        certificate_available_date=cert_downloadable_status.get(
            "certificate_available_date"
        ),
    )
...
def load_metadata_from_youtube(video_id, request):
    ...
    if (
        ...
        and settings.YOUTUBE_API_KEY != "PUT_YOUR_API_KEY_HERE"
    ):
        yt_api_key = settings.YOUTUBE_API_KEY
        yt_metadata_url = settings.YOUTUBE["METADATA_URL"]
        yt_timeout = (
            settings.YOUTUBE.get("TEST_TIMEOUT", 1500) / 1000
        ...
        try:
            res = requests.get(
                yt_metadata_url, params=payload, timeout=yt_timeout, headers=headers
            )
            ...
            if res.status_code == 200:
                try:
                    ...
                    if res_json.get("items", []):
                        ...
                        logging.warning(
                            "Unable to find the items in response. Following response "
                            "was received: {res}".format(res=res.text)
                        )
                except ValueError:
                    logging.warning(
                        "Unable to decode response to json. Following response "
                        "was received: {res}".format(res=res.text)
                    )
            else:
                logging.warning(
                    "YouTube API request failed with status code={status} - "
                    "Error message is={message}".format(
                        status=status_code, message=res.text
                    )
                )
...
def course_info(request, course_id):
    ...
    with modulestore().bulk_operations(course_key):
        ...
        masquerade, user = setup_masquerade(
        ...
        course_homepage_show_subtitle = configuration_helpers.get_value(
            "COURSE_HOMEPAGE_SHOW_SUBTITLE", True
        )

        course_homepage_show_org = configuration_helpers.get_value(
        ...
        # TODO: LEARNER-611: Remove enable_course_home_improvements
        if SelfPacedConfiguration.current().enable_course_home_improvements:
            context["resume_course_url"] = get_last_accessed_courseware(
                course, request, user
            )
...
class StaticCourseTabView(EdxFragmentView):
    ...
    def get(
        self, request, course_id, tab_slug, **kwargs
    ...
    def render_to_fragment(
        self, request, course=None, tab=None, **kwargs
    ):  # lint-amnesty, pylint: disable=arguments-differ
        """
    ...
    def render_standalone_response(
        self, request, fragment, course=None, tab=None, **kwargs
    ):  # lint-amnesty, pylint: disable=arguments-differ
        """
...
class CourseTabView(EdxFragmentView):
        ...
        with modulestore().bulk_operations(course_key):
            ...
            try:
                ...
                page_context = self.create_page_context(
                    request, course=course, tab=tab, **kwargs
                )
                ...
                return super().get(
                    request, course=course, page_context=page_context, **kwargs
                )
            except Exception as exception:  # pylint: disable=broad-except
                return CourseTabView.handle_exceptions(
                    request, course_key, course, exception
                )
        ...
        if request.user.is_anonymous and not allow_anonymous:
            if CourseTabView.course_open_for_learner_enrollment(course):
                PageLevelMessages.register_warning_message(
                    request,
                    Text(
                        _("To see course content, {sign_in_link} or {register_link}.")
                    ).format(
                        sign_in_link=HTML(
            ...
            if (
                not CourseEnrollment.is_enrolled(request.user, course.id)
    ...
    def course_open_for_learner_enrollment(course):
        return (
            course_open_for_self_enrollment(course.id)
    ...
    def handle_exceptions(request, course_key, course, exception):
        ...
        """
        if isinstance(exception, Redirect) or isinstance(
            exception, Http404
    ...
    def create_page_context(self, request, course=None, tab=None, **kwargs):
        ...
        if "profile_page_context" in kwargs:
            context["load_mathjax"] = kwargs["profile_page_context"].get(
                "load_mathjax", True
            )
    ...
    def render_to_fragment(
        self, request, course=None, page_context=None, **kwargs
    ):  # lint-amnesty, pylint: disable=arguments-differ
        """
    ...
    def render_standalone_response(
        self, request, fragment, course=None, tab=None, page_context=None, **kwargs
    ):  # lint-amnesty, pylint: disable=arguments-differ
        """
    ...
    def get(self, request, course_id):
        ...
        with modulestore().bulk_operations(course_key):
            ...
            if not registered_for_course(course, user):
                context = {"course": course, "csrftoken": csrf(request)["csrf_token"]}
...
def dates(request, course_id):
    ...
    if not (course_home_legacy_is_active(course_key) or request.user.is_staff):
        raise Redirect(
            get_learning_mfe_home_url(
                ...
                url_fragment=COURSE_DATES_NAME,
                params=request.GET,
            )
        )
    ...
    course_date_blocks = get_course_date_blocks(
        course, request.user, request, include_access=True, include_past_dates=True
    )
    ...
    context = {
        ...
        "verified_upgrade_link": verified_upgrade_deadline_link(
...
def progress(request, course_id, student_id=None):
    ...
    if course_home_mfe_progress_tab_is_active(course_key) and not request.user.is_staff:
        ...
        raise Redirect(
            get_learning_mfe_home_url(
                course_key=course_key,
                url_fragment=end_of_redirect_url,
                params=request.GET,
            )
        )
...
def _downloadable_certificate_message(
    course, cert_downloadable_status
...
def _missing_required_verification(student, enrollment_mode):
    return not settings.FEATURES.get("ENABLE_INTEGRITY_SIGNATURE") and (
        enrollment_mode in CourseMode.VERIFIED_MODES
        and not IDVerificationService.user_is_verified(student)
    )


def _certificate_message(
    student, course, enrollment_mode
):  # lint-amnesty, pylint: disable=missing-function-docstring
...
def get_cert_data(student, course, enrollment_mode, course_grade=None):
    ...
    if not CourseMode.is_eligible_for_certificate(
        enrollment_mode, status=cert_data.cert_status
    ):
    ...
    certificates_enabled_for_course = certs_api.has_self_generated_certificates_enabled(
        course.id
    )
    if course_grade is None:
        course_grade = CourseGradeFactory().read(student, course)

    if not certs_api.can_show_certificate_message(
        course, student, course_grade, certificates_enabled_for_course
    ):
        return

    if not certs_api.get_active_web_certificate(
        course
    ) and not certs_api.is_valid_pdf_certificate(cert_data):
        return
...
def credit_course_requirements(course_key, student):
    ...
    if not (
        ...
        and is_credit_course(course_key)
    ):
        return None
    ...
    # then the user is NOT eligible for credit.
        ...
        requirement["status"] in non_eligible_statuses
...
def _course_home_redirect_enabled():
    ...
    if configuration_helpers.get_value(
        ...
        settings.FEATURES.get("ENABLE_COURSE_HOME_REDIRECT", True),
    ):
        return True
...
def submission_history(request, course_id, learner_identifier, location):
    ...
    try:
        history_entries = list(
            user_state_client.get_history(found_user_name, usage_key)
        )
    except DjangoXBlockUserStateClient.DoesNotExist:
        return HttpResponse(
            escape(
                _("User {username} has never accessed problem {location}").format(
                    username=found_user_name, location=location
                )
            )
        )
    ...
    if len(scores) != len(history_entries):
        ...
        scores_by_date = {score.created: score for score in scores}
        scores = [scores_by_date[history.updated] for history in history_entries]
...
def get_static_tab_fragment(request, course, tab):
    ...
    if tab_module is not None:
        try:
            ...
            log.exception("Error rendering course=%s, tab=%s", course, tab["url_slug"])
...
def get_course_lti_endpoints(request, course_id):
    ...
    lti_descriptors.extend(
        modulestore().get_items(course.id, qualifiers={"category": "lti_consumer"})
    )
    ...
    lti_noauth_modules = [
        get_module_for_descriptor(
            ...
            FieldDataCache.cache_for_descriptor_descendents(
                course_key, anonymous_user, descriptor
            ),
            course_key,
            course=course,
        )
    ...
    )  # lint-amnesty, pylint: disable=http-response-with-content-type-json, http-response-with-json-dumps
...
def course_survey(request, course_id):
    ...
    course_key = CourseKey.from_string(course_id)
    course = get_course_with_access(
        request.user, "load", course_key, check_survey_complete=False
    )
...
def generate_user_cert(request, course_id):
    ...
    log.info(
        f"Attempt will be made to generate a course certificate for {student.id} : {course_key}."
    )
...
def render_xblock(request, usage_key_string, check_if_enrolled=True):
    ...
    if (
        requested_view != "student_view" and requested_view != "public_view"
    ):  # lint-amnesty, pylint: disable=consider-using-in
        return HttpResponseBadRequest(
    ...
    with modulestore().bulk_operations(course_key):
        ...
        try:
            course = get_course_with_access(
                request.user, "load", course_key, check_if_enrolled=check_if_enrolled
            )
        ...
        student_view_context["show_bookmark_button"] = (
            request.GET.get("show_bookmark_button", "0") == "1"
        )
        ...
        student_view_context["hide_access_error_blocks"] = (
            is_learning_mfe and recheck_access
        )
            ...
            if ancestor_sequence_block.descendants_are_gated(context):
                return redirect(
                    reverse(
                        "render_xblock",
                        kwargs={
                            "usage_key_string": str(ancestor_sequence_block.location)
                        },
                    )
                )
        ...
        context = {
            ...
            "staff_access": staff_access,
            "xqa_server": settings.FEATURES.get(
...
def get_optimization_flags_for_content(block, fragment):
    ...
    """
    safe_defaults = {"enable_mathjax": True}
...
FA_SHORT_ANSWER_INSTRUCTIONS = _(
    "Use between 1250 and 2500 characters or so in your response."
)
...
def financial_assistance_request(request):
    ...
    zendesk_submitted = create_zendesk_ticket(
        ...
        "Financial assistance request for learner {username} in course {course_name}".format(
            username=username, course_name=course.display_name
        ),
    ...
    if not (
        zendesk_submitted == 200 or zendesk_submitted == 201
    ):  # lint-amnesty, pylint: disable=consider-using-in
...
def financial_assistance_form(request):
    ...
    annual_incomes = [
        {"name": _(income), "value": income}
        for income in incomes  # lint-amnesty, pylint: disable=translation-of-non-string
    ]
    return render_to_response(
...
def get_financial_aid_courses(user):
    ...
    for enrollment in CourseEnrollment.enrollments_for_user(user).order_by("-created"):
        ...
        if (
            enrollment.mode != CourseMode.VERIFIED
            and enrollment.course_overview
            and enrollment.course_overview.eligible_for_financial_aid
            and CourseMode.objects.filter(
                Q(_expiration_datetime__isnull=True)
                | Q(_expiration_datetime__gt=datetime.now(UTC)),
...
def get_learner_username(learner_identifier):
    ...
    learner = User.objects.filter(
        Q(username=learner_identifier) | Q(email=learner_identifier)
    ).first()
    if learner:
